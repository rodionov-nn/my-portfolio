...
function ShaderPlane() {
    ...
    useFrame(({ clock, size }) => {
        if (material.current) {
            ...
            material.current.uniforms.uResolution.value = new THREE.Vector2(size.width, size.height);
            material.current.uniforms.uSpeed.value = 0.5; // Значение от 0.1 (очень медленно) до 1.0 (оригинальная скорость)
            material.current.uniforms.uGrainSize.value = 2.5; // Значение размера шума, можно сделать пропсом
        }
    });
        ...
        <mesh ref={mesh}>
            ...
            <shaderMaterial
                ...
                uniforms={{
                    ...
                    uSpeed: { value: 0.5 }, // Начальное значение скорости
                    uGrainSize: { value: 2.5 } // Начальное значение размера шума
                }}
